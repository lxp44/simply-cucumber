...
import { CheckCircle2, Leaf, FlaskConical, Sparkles, Droplets, Snowflake, Sun, ShieldCheck } from "lucide-react";
...
export default function ProductDetailMobile({
  ...
  // ✅ Build gallery: prefer variant.images[], then variant.image, then product.images
  const imgs = useMemo(() => {
  ...
  // ✅ when variant changes, jump to first slide
  useEffect(() => { setActive(0); }, [vIdx]);
  ...
  function addToCart() {
    const payload = {
      ...
      title: `${product.title}${(selectedVariant as any).label ? ` — ${(selectedVariant as any).label}` : ""}`,
      price: unitPrice,
      quantity: qty,
      image: imgs[0],
