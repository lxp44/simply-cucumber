...
// 👇 use your existing Netlify env names
const {
  GMAIL_USER,             // your Gmail address
  GMAIL_APP_PASS,         // 16-char app password
  CONTACT_TO,             // where owner notification goes
  SITE_URL = "https://www.simplycucumber.com",
} = process.env;

const BRAND_NAME = "Simply Cucumber";
const BRAND_FROM_NAME = "Simply Cucumber Support";
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"${BRAND_FROM_NAME}" <${GMAIL_USER}>`,
